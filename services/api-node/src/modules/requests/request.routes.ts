...
requestRouter.get("/:id", async (req, res, next) => {
  try {
    ...
    if (userRole && (userRole === "student" || userRole === "staff" || userRole === "user")) {
...
requestRouter.patch("/:id/status", async (req, res, next) => {
  try {
    const userRole = req.headers["x-user-role"] as string | undefined;
    const userEmail = req.headers["x-user-email"] as string | undefined;

    if (userRole !== "admin") {
      throw new HttpError(403, "เฉพาะผู้ดูแลระบบเท่านั้นที่เปลี่ยนสถานะได้");
    }
...
requestRouter.patch("/:id/reply", async (req, res, next) => {
  try {
    const userRole = req.headers["x-user-role"] as string | undefined;
    const userEmail = req.headers["x-user-email"] as string | undefined;

    if (userRole !== "admin") {
      throw new HttpError(403, "เฉพาะผู้ดูแลระบบเท่านั้นที่ตอบกลับได้");
    }
    ...
    const updated = await requestService.reply(req.params.id, reply, userEmail);
    res.json(updated);
  } catch (error) {
    next(error);
  }
